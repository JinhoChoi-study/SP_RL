        ...
        try:
            for step in range(num_step):
               ...
               #======================== ready for action, state vector=============================================
                ...
                ttc, _ = compute_ttc(ego_vehicle, nearest_vehicles)
                ttc_margin = 2.5
                penalty_scale = 0.1  

                if ttc < ttc_margin:
                    ttc_penalty = -(ttc_margin - ttc) * penalty_scale
                else:
                    ttc_penalty = 0.0

                ttc_penalty_total += ttc_penalty
                ...
                reward = ttc_penalty + speed_reward
                ...
                if not done and ttc < 1.5 :
                    acc_penalty = ttc_penalty * action
                    reward += acc_penalty
